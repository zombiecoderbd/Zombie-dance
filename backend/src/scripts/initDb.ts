...
const MIGRATION_PATH = path.join(__dirname, "../../../scripts/database_migration.sql");
 ...
 */
async function importOllamaModels(db: Database.Database): Promise<number> {
 ...
 */
function verifyDatabase(db: Database.Database): boolean {
 ...
 */
function getDatabaseStats(db: Database.Database): any {
    console.log("\n📊 Database Statistics:");
    ...
    try {
        const stats = {
            ...
            models:
                (db.prepare("SELECT COUNT(*) as count FROM model_configs").get() as any)?.count ||
                0,
            sessions:
                (db.prepare("SELECT COUNT(*) as count FROM chat_sessions").get() as any)?.count ||
                0,
            messages:
                (db.prepare("SELECT COUNT(*) as count FROM chat_messages").get() as any)?.count ||
                0,
            activeModels:
                (
                    db
                        .prepare("SELECT COUNT(*) as count FROM model_configs WHERE is_active = 1")
                        .get() as any
                )?.count || 0,
        };
 ...
 */
function listModels(db: Database.Database): void {
    console.log("\n🎯 Available Models:");
