...
const wss = new WebSocketServer({
    ...
    verifyClient: (info: { origin: string; secure: boolean; req: any }) => {
        ...
        const isVSCode = userAgent.includes("vscode") || userAgent.includes("Visual Studio Code");
...
app.use(
    cors({
        origin: function (origin, callback) {
            ...
            // Allow vscode origins
            if (origin.includes("vscode")) return callback(null, true);
...
app.use("/v1/chat", openaiRouter); // OpenAI-compatible endpoints
app.use("/v1", openaiRouter); // OpenAI models endpoint
...
app.use("*", (req, res) => {
    ...
    res.status(404).json({
        ...
        troubleshooting: isVSCode
            ? {
                  extension_setup: "Ensure ZombieCoder extension is properly configured",
